...
/*			var usedVersion = semver.clean(versionRange);

			if (!cached || !cached.version) {
				return console.error('Couldn\'t find package ' + packageName + ' or it\'s version in cache');
			}

			if (!usedVersion) {
				return console.error('Couldn\'t find expected version for ' + packageName + ' in ' + subscription.repourl + ' !!!  versionRange ' + versionRange + ' usedVersion ' + usedVersion);
			}

			if(semver.lte(cached.version, usedVersion) ||
*/
			if(!(semver.satisfies(cached.version, versionRange) || semver.gtr(cached.version, versionRange)) ||
