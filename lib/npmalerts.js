...
function checkUpdates(callback) {
  ...
  function onVersions(error, versions) {
    ...
    function findDependencies(repo) {
      ...
      Object.keys(repo.dependencies).forEach(checkDepVersionFor(repo.dependencies, newPackages[repo].dependencies));
      Object.keys(repo.devDependencies).forEach(checkDepVersionFor(repo.devDependencies, newPackages[repo].devDependencies));
    ...
    Object.keys(emails).forEach(processEmail);

    function processEmail(email) {
      emails[email].forEach(sendEmail);

      function sendEmail(repo) {
        ...
        if (Object.keys(newPacks.dependencies).length || Object.keys(newPacks.devDependencies).length) {
          ...
          emailWrapper.send(email, repourl, newPacks);
        }
      }
    }
