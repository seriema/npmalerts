...
db.readSubscriptions(function(error, subscriptions) {
	...
	for(var i = 0, n = subscriptions.length; i < n; i++) {
		...
		if (!user) {
			...
			github.getPackageJson(user, repo, processPackage(subscription));
		}
	}
});
...
function checkPackage(emailAdress, repourl, packageName, expectedRange) {
	getLatestVersion(packageName, function(error, latestVersion, isNew) {
		if (error) {
			console.error(error);
			return;
		}
		if (isNew && semver.satisfies(latestVersion, expectedRange)) {
			email.send(emailAdress, repourl, packageName, expectedRange, latestVersion);
		}
	});
}
...
function processPackage(subscription) {
