    ...
    function findDependencies(repo) {
      ...
      function checkDepVersionFor(dep, result) {
        return function (package) {
          console.info('%s: Date %s >= %s is %s, isNew(%s, %s) is %s', package, versions[package].updated, yesterday, versions[package].updated >= yesterday, versions[package].latest, dep[package], semver.isNew(versions[package].latest, dep[package]));
//          console.info('Package %s, latest %s, used %s', package, versions[package].latest, dep[package]);
    ...
    function processEmail(email, callback) {
      ...
      function sendEmail(key) {
        emails[key].forEach(function (repo) {
          ...
          console.info('Sending email to ' + key + ' for ' + repourl + ' about potentially ' + numNewPacks + ' packages.');
