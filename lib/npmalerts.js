    ...
    function findDependencies(repo) {
      ...
      function checkDepVersionFor(dep, result) {
        return function (pack) {
          console.info('%s: Date %s >= %s is %s, isNew(%s, %s) is %s', pack, versions[pack].updated, yesterday, versions[pack].updated >= yesterday, versions[pack].latest, dep[pack], semver.isNew(versions[pack].latest, dep[pack]));
    ...
    function processEmail(email, callback) {
      ...
      function sendEmail(key) {
        emails[key].forEach(function (repo) {
          ...
          console.info('Email. Key: %s, repo: %s, new packages: %s', key, repo, numNewPacks);
          if (numNewPacks === 0)
            return callback();
