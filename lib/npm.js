...
function getLatestVersions(names, callback) {
  ...
  asyncMap(names, function (name, callback) {
    npm.registry.get(name, function (err, data) { // TODO: This is an internal function and not meant to be called. Transition to https://www.npmjs.org/package/npm-registry-client using client.get().
      if (err) {
        callback(err);
        return;
      }
